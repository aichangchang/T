	...
	<div class="main">
        ...
        <ul v-if="this.FuelDispenserData.length>0">
          <li v-for="(item, index) in FuelDispenserData" class="goods_list" :key="index">
            <router-link :to="{ name: 'Details', params: { detail: item }}">
              <a><img v-bind:src="item" /></a>
              <div class="proTitle" v-for="(items, index) in FuelDispenserDataTitle">{{items.title}}</div>
            </router-link>
          </li>
			  </ul>
			</div>
    ...
    methods:{
      ...
      change: function (index, text) {
        ...
        if(text == 'System'){
          ...
          this.FuelDispenserDataTitle = [];
          FuelDispenserDatas.map((item, index)=>{
            item.src.map((items, indexs)=>{
              if(indexs==0){
                this.FuelDispenserData.push(items.src)
              }
            })
            item.technicalTitle.map((items, indexs)=>{
              if(indexs==0){
                this.FuelDispenserDataTitle.push(items.title)
              }
            })
            this.FuelDispenserData.concat(this.FuelDispenserDataTitle)
            console.log(this.FuelDispenserData.concat(this.FuelDispenserDataTitle))
          })
