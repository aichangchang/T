<template>
	<div class="main">
        ...
        <ul v-if="this.FuelDispenserData.length>0">
          <li class="goods_list">
            <router-link v-for="(item, index) in FuelDispenserData" :key="index" :to="{ name: 'Details', params: { detail: item }}">
              <a><img v-bind:src="item" /></a>
            </router-link>
            <div class="proTitle" v-for="(items, index) in FuelDispenserDataTitle" :key="index">{{items}}</div>
          </li>
			  </ul>
			</div>
		</div>
