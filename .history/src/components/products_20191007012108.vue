<template>
	<div class="main">
        ...
        <ul v-if="this.FuelDispenserData.length>0">
          <li class="goods_list" :key="index">
            <router-link v-for="(item, index) in FuelDispenserData" :to="{ name: 'Details', params: { detail: item }}">
              <a><img v-bind:src="item" /></a>
            </router-link>
            <div class="proTitle" v-for="(items, index) in FuelDispenserDataTitle">{{items}}</div>
          </li>
			  </ul>
			</div>
		</div>
        ...
        if(text == 'System'){
          ...
          FuelDispenserDatas.map((item, index)=>{
            item.src.map((items, indexs)=>{
              if(indexs==0){
                this.FuelDispenserData.push(items)
              }
            })
