<template>
	<div class="main">
        ...
        <ul v-if="this.FuelDispenserSrcData.length>0">
          <li v-for="(item, index) in FuelDispenserTitleData" class="goods_list" :key="item.src">
            <img v-bind:src="item.src" />
          </li>
          <li v-for="(item, index) in FuelDispenserSrcData" class="goods_list" :key="item.src">
            <p>{{item.title}}</p>
          </li>
			  </ul>
			</div>
    ...
    methods:{
      ...
      detailGet(id, title){
        if(id == 'System'){
          ...
          console.log(this.FuelDispenserSrcData, this.FuelDispenserTitleData)
